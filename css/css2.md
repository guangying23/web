...
## 1.8 定位叠放次序z-index
```
选择器{z-index:1;}    数值越大（不能加单位），盒子越靠上，数值相同时按照书写顺序后来者居上
```
z-index 属性指定一个元素的堆叠顺序。<br>
拥有更高堆叠顺序的元素总是会处于堆叠顺序较低的元素的前面。<br>
注意： z-index 进行定位元素(position:absolute, position:relative, or position:fixed)。<br>
...
## 2.2 绝对定位的盒子居中
- 无法用margin水平居中
- `left:50%;-0.5*width 0f absolute box;`
## 2.3 定位特殊特性 
绝对定位和固定定位也和浮动类似
...
## 2.4 脱标的盒子不会触发外边距合并
浮动元素、绝对定位（固定定位）元素都不会触发外边距合并

## 2.5 绝对定位（固定定位）会完全压住盒子
浮动元素不同，只会压住它下面标准流的盒子，但是不会压住下面标准流盒子里面的文字（图片）<br>
绝对定位（固定定位）会完全压住下面标准流所有内容<br>
浮动之所以不会压住文字，因为浮动产生的目的最初是为了做文字环绕，文字会围绕浮动元素。
## 2.6 偏移
同时定义了left right 先左后右，先上后下
